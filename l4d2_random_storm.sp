...
#define CONFIG_SETTINGS		"data/l4d2_storm_random.cfg"
#define CONFIG_TRIGGERS		"data/l4d2_storm_triggers_random.cfg"

#define MODEL_BOUNDING		"models/props/cs_militia/silo_01.mdl"           // something wrong with this one..
...
static 	Handle:g_hCvarAllow, Handle:g_hCvarModes, Handle:g_hCvarModesOff, Handle:g_hCvarModesTog, Handle:g_hCvarMixer, Handle:g_hCvarPost, Handle:g_hCvarStyle,
		...
		// Other data settings
		String:g_sSkyBox[64], g_iCfgBackground, String:g_sCfgLightStyle[64], g_iCfgClouds, g_iCfgRain, g_iCfgRainIdle, g_iCfgRainStorm, g_iCfgWind,
		g_iCfgLight, g_iCfgLightDmg, g_iCfgLightTime, g_iCfgLightFlash,
		...
		g_iLight, g_iRains[MAX_RAIN], g_iWind, g_iStormLayer, g_iVoip, g_iVoipIn, g_iVoipOut, g_iPointCmd, g_iLogicDirector, g_iFogVolume,
		g_iPostProcess, g_iLogicIn, g_iLogicOut, g_iSound, g_iSkyCamera, g_iSkyCam[2];

static  Handle: g_hCvarFogMode = INVALID_HANDLE;    // Tab-added
...
{
	name = "[L4D2] Weather Control [modified for use with random]",
	author = "SilverShot (ed. by Tabun)",
...
public OnPluginStart()
{
	...
	g_hCvarAllow = CreateConVar(		"l4d2_storm_allow",			"0",			"0=Plugin off, 1=Plugin on.", CVAR_FLAGS);
	...
	CreateConVar(						"l4d2_storm_version",		PLUGIN_VERSION,	"Weather Control plugin version.", CVAR_FLAGS|FCVAR_REPLICATED|FCVAR_DONTRECORD);
    ...
    g_bCvarAllow = GetConVarBool(g_hCvarAllow);     // tab-added
}
...
IsAllowed()
{
    //PrintToServer("[storm] IsAllowed: %i (cvar %i)", g_bCvarAllow, GetConVarBool(g_hCvarAllow));
...
ConfigJumpA(Handle:hFile, String:sMap[64])
{
	...
	if( KvJumpToKey(hFile, sMap) == true || KvJumpToKey(hFile, "main") == true )
	{
        ...
        /*
            Tabun
            hardcoded easy tweak for 'random' config
            if we're in 'fog-mode', change what's actually in the config
...
CreateRain()
{
	...
	for( new i = 0; i < g_iCfgRain; i++ )
	{
		...
		if( entity != -1 )
		{
			...
			LogError("Failed to create %d 'func_precipitation'", i+1);
	}
}
