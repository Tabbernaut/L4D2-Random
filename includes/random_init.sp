...
{
    // ConVars
    g_hCvarDebug = CreateConVar(                            "rand_debug",                    "2",       "Random debug mode. (0: only error reporting, -1: disable all reports, 1+: set debug report level)", FCVAR_NONE, true, -1.0);
    g_hCvarConfogl = CreateConVar(                          "rand_confogl",                  "1",       "Whether random is loaded as a confogl matchmode (changes the way cvar defaults are read).", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarLoaded = CreateConVar(                           "rand_cvars_loaded",             "0",       "For checking whether the config files are all properly loaded. ", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarSimplePauseCheck = CreateConVar(                 "rand_simplepausecheck",         "1",       "Uses sv_pausable for a simple pause check.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarStripperMode = CreateConVar(                     "rand_stripper_mode",            "2",       "When using Stripper:Source: mode 0 = don't change dir; 1 = toggle standard and _alt (50%); 2 = standard + _alt (33%); 3 = same, but (25%).", FCVAR_NONE, true, 0.0, true, 2.0);
    g_hCvarStripperPath = CreateConVar(                     "rand_stripper_path",            "addons/stripper",             "The Stripper:Source directory random uses as its base.", FCVAR_NONE);
    g_hCvarRIKeyValuesPath = CreateConVar(                  "rand_randominfo_path",          "configs/randommapinfo.txt",   "The path to the randommap.txt with keyvalues for per-map random settings.", FCVAR_NONE);
    g_hCvarRCKeyValuesPath = CreateConVar(                  "rand_randomconfig_path",        "configs/randomconfig.txt",    "The path to the randomconfig.txt with keyvalues for its base settings.", FCVAR_NONE);
    g_hCvarWelcomeMode = CreateConVar(                      "rand_welcome",                  "3",       "Whether to display welcome messages (1 = only in first round; 2 = always, 3 = each client only once).", FCVAR_NONE, true, 0.0, true, 3.0);
    g_hCvarBlockL4D1Common = CreateConVar(                  "rand_no_l4d1_common",           "3",       "Whether to block L4D1 common. (2 = block all appearing l4d1 common; 3 = block only problematic skins)", FCVAR_NONE, true, 0.0, true, 3.0);
    g_hCvarNerfT2 = CreateConVar(                           "rand_nerf_t2",                  "1",       "Whether to nerf t2 weapons (disallow anything better than single pistol for secondary)", FCVAR_NONE, true, 0.0, true, 2.0);
    g_hCvarStopBotsAtStart = CreateConVar(                  "rand_stop_bots",                "1",       "Whether to stop bots from doing anything before humans are ready.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarBlockEventVotes = CreateConVar(                  "rand_block_event_votes",        "0",       "Whether to block event votes.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarUseOldSpawn = CreateConVar(                      "rand_use_old_spawn",            "1",       "Whether to use z_spawn_old rather than z_spawn.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarCampaignStreak = CreateConVar(                   "rand_campaign_streak",          "3",       "How many times a map should be tried before the event changes (0 = never changes).", FCVAR_NONE, true, 0.0, false);

    g_hCvarEqual = CreateConVar(                            "rand_equal",                 "2047",       "[Flags] What to keep equal between each team's survivor round (1: items; 2: doors; 4: glows; 8: event; 16: incaps; 32: horde; 64: item weighting; 128: starting health; 256: first attack; 512: tanks; 1024: scoring).", FCVAR_NONE, true, 0.0, false);
    g_hCvarDoReport = CreateConVar(                         "rand_report",                   "1",       "Whether to do automatic reports at the start of a round.", FCVAR_NONE, true, 0.0, true, 1.0 );
    g_hCvarReportDelay = CreateConVar(                      "rand_report_delay",            "15",       "How many seconds after first survivor joins map to wait before reporting special event.", FCVAR_NONE, true, 1.0, true, 120.0 );
    g_hCvarReportSackProt = CreateConVar(                   "rand_report_sackprotection",    "2",       "Whether sack-protection measures are reported to the relevant players (1: always; 2: only to offending player once)", FCVAR_NONE, true, 0.0, true, 2.0 );
    g_hCvarRandomSpawns = CreateConVar(                     "rand_random_si",                "1",       "Whether SI spawns are fully random (or Valve-ordered).", FCVAR_NONE, true, 0.0, true, 1.0 );
    g_hCvarSackProtection = CreateConVar(                   "rand_si_sackprotection",        "1",       "Whether SI spawn sacking is punished (keeping a charger hoping to get a multi-charger attack, for instance).", FCVAR_NONE, true, 0.0, true, 1.0 );
    g_hCvarDeathOrderMode  = CreateConVar(                  "rand_si_deathorder",            "1",       "Death order mode: 0 = totally random; 1 = 3-spawn timeout with 60% repick rate; 2 = 4-spawn timeout with 100% repick rate.", FCVAR_NONE, true, 0.0, true, 2.0 );
    g_hCvarDifficultyBalance = CreateConVar(                "rand_difficulty_balance",       "1",       "Whether round settings should be balanced according to estimated difficulty).", FCVAR_NONE, true, 0.0, true, 1.0 );
    g_hCvarM60Ammo = CreateConVar(                          "rand_m60_ammo",                "50",       "How much ammo an M60 spawns with.", FCVAR_NONE, true, 0.0, false );
    ...
    g_hCvarBoomedTime = CreateConVar(                       "rand_boomed_time",             "10.0",     "The time window in seconds that boomer team-ups are rewarded (2+ boomers getting booms on different survivors). 0 = boomer combos are not rewarded.", FCVAR_NONE, true, 0.0, true, 20.0);
    g_hCvarGnomeBonus = CreateConVar(                       "rand_gnome_bonus",              "0.2",     "The bonus given for bringing a gnome from start to end saferoom. (lower than 10 = amount of times distance, greater = static bonus)", FCVAR_NONE, true, 0.0);
    g_hCvarGnomeFinaleFactor = CreateConVar(                "rand_gnome_finale_factor",      "0.5",     "The gnome bonus is worth this factor on finales.", FCVAR_NONE, true, 0.0);
    g_hCvarGnomeAllowRandom = CreateConVar(                 "rand_gnome_random",             "0",       "Whether gnomes can drop at random (from gifts, common drops etc)", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarSpecialEventTimeout = CreateConVar(              "rand_event_timeout",            "5",       "How many maps must be played before the same special event may be picked again.", FCVAR_NONE, true, 0.0, false);
    g_hCvarBanTankFlows = CreateConVar(                     "rand_ban_tanks",                "1",       "Whether tank flow bans will be taken into account.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarRandomHittables = CreateConVar(                  "rand_hittables",                "1",       "Whether hittables should be randomized too.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarMiniTankHealth = CreateConVar(                   "rand_minitankhealth",        "1000",       "How much health minitanks have. This is two-thirds of its versus health (900 = 1350 in versus).", FCVAR_NONE, true, 500.0, false);
    g_hCvarDoubleTankHealth = CreateConVar(                 "rand_doubletankhealth",      "3000",       "How much health each tank on a double-tank map has. (3000 = 4500 in versus).", FCVAR_NONE, true, 1000.0, false);

    g_hCvarNoitemVariance = CreateConVar(                   "rand_noitem_variance",          "0.25",    "Variance of weight for 'no item' in item randomizer.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarSpecialEventChance = CreateConVar(               "rand_event_chance",             "0.65",    "Chances of any map going in 'special event' mode.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarPillsChance = CreateConVar(                      "rand_pills_chance",             "0.45",    "Chance that a survivor is given pills at start.", FCVAR_NONE, true, 0.0, true, 1.0 );
    g_hCvarPistolChance = CreateConVar(                     "rand_pistol_chance",            "0.9",     "Chance that a survivor is given at least a pistol as secondary at start.", FCVAR_NONE, true, 0.0, true, 1.0 );
    g_hCvarExtraSecondaryChance = CreateConVar(             "rand_secondary_chance",         "0.5",     "Chance that a survivor is given a secondary weapon if given a primary at start.", FCVAR_NONE, true, 0.0, true, 1.0 );
    g_hCvarHealthChance = CreateConVar(                     "rand_health_chance",            "0.0",     "Chance that a survivor is given different starting health.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarHealthMin = CreateConVar(                        "rand_health_min",              "50",       "Minimum survivor starting health.", FCVAR_NONE, true, 1.0, true, 100.0);
    g_hCvarOutlineChance = CreateConVar(                    "rand_outline_chance",           "0.85",    "Chances of there being survivor outlines this round.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarDoorLockedChance = CreateConVar(                 "rand_doorlocked_chance",        "0.1",     "Chances of any door being locked.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarStartSafeItem = CreateConVar(                    "rand_startsafe_item_chance",    "1.0",     "Chances that an item spawn in the start saferoom will be used.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarEndSafeItem = CreateConVar(                      "rand_endsafe_item_chance",      "0.0",     "Chances that an item spawn in the end saferoom will be used.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarUncommonChance = CreateConVar(                   "rand_uncommon_chance",          "0.05",    "Chances of any common spawning becoming an uncommon.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarFallenChance = CreateConVar(                     "rand_fallen_chance",            "0.05",    "Chances of any uncommon spawning becoming a Fallen Survivor.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarJimmyChance = CreateConVar(                      "rand_jimmy_chance",             "0.003",   "Chances of any uncommon spawning becoming Jimmy Gibbs.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarFirstQuadChance = CreateConVar(                  "rand_firstquad_chance",         "0.1",     "Chances of the first attack being a quad cap.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarAmmoVarianceMore = CreateConVar(                 "rand_ammo_more_variance",       "0.15",    "Variance of ammo in weapon pickups, positive direction.", FCVAR_NONE, true, 0.0, false);
    g_hCvarAmmoVarianceLess = CreateConVar(                 "rand_ammo_less_variance",       "0.2",     "Variance of ammo in weapon pickups, negative direction.", FCVAR_NONE, true, 0.0, false);
    g_hCvarDoubleTankChance = CreateConVar(                 "rand_doubletank_chance",        "0.0",     "Chances of double tanks spawning for a round that will have a tank.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarMultiWitchChance = CreateConVar(                 "rand_multiwitch_chance",        "0.4",     "Chances of multiple witches spawning for a round that will have a witch.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarTankFlowVariance = CreateConVar(                 "rand_tankflow_variance",        "0.1",     "Amount of variance allowed for multiple tank spawns (around 0.25 and 0.75).", FCVAR_NONE, true, 0.0, true, 0.20);
    g_hCvarItemDropChance = CreateConVar(                   "rand_itemdrop_chance",          "0.01",    "Chances of common infected dropping an item.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarTankItemDropChance = CreateConVar(               "rand_tankitemdrop_chance",      "0.75",    "Chances of a (normal) tank dropping some items.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarGiftPositiveChance = CreateConVar(               "rand_giftgood_chance",          "0.63",    "Chances of opening a gift resulting in something positive.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarPipeDudChance = CreateConVar(                    "rand_pipedud_chance",           "0.35",    "Chances of a pipebomb being a dud.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarAvoidIncapsChance = CreateConVar(                "rand_moreincaps_chance",        "0.35",    "If the incap count is only 1 (33%), odds that it gets set to 2 anyway.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarNormalAmmoChance = CreateConVar(                 "rand_normal_ammo",              "0.65",    "Chances that ammo piles are randomized (if an event doesn't override this).", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarFinaleAmmoChance = CreateConVar(                 "rand_finale_ammo",              "0.0",     "Chances of finale ammo piles being randomized.", FCVAR_NONE, true, 0.0, true, 1.0);
    ...
    g_hCvarStartItemAmmo =  CreateConVar(                   "rand_item_start_ammo",          "0.5",     "Chances that there will be at least one ammo pile in the start saferoom (if non are generated there).", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarStartItemGnome =  CreateConVar(                  "rand_item_start_gnome",         "0.25",    "Chances that there will be at least one gnome in the start saferoom (if non are generated there).", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarStartBalanceSurv = CreateConVar(                 "rand_item_start_balance_surv",  "1",       "If enabled, survivor handouts are adjusted based on the round's difficulty rating.", FCVAR_NONE, true, 0.0, true, 1.0);
    g_hCvarSpawnBalanceMode = CreateConVar(                 "rand_spawn_balance_mode",       "1",       "Balance chargers, spitters and boomers (ex. first attack): 0 = off; 1 = reduce odds when already 2 in the attack; 2 = more strict; 3 = block more than 2; 4 = block more than 1.", FCVAR_NONE, true, 0.0, true, 5.0);

    g_hCvarRandDistance = CreateConVar(                     "rand_distance",                 "1",       "Distance mode: 0 = normal; 1 = variance from normal; 2 = full random.", FCVAR_NONE, true, 0.0, true, 2.0);
    g_hCvarRandDistVar = CreateConVar(                      "rand_dist_var",                 "0.2",     "For distance variance: how much it can deviate from normal value.", FCVAR_NONE, true, 0.1, true, 0.9);
    g_hCvarRandDistMin = CreateConVar(                      "rand_dist_min",               "200",       "For fully random distance: the minimum.", FCVAR_NONE, true, 50.0, true, 900.0);
    g_hCvarRandDistMax = CreateConVar(                      "rand_dist_max",              "1000",       "For fully random distance: the maximum.", FCVAR_NONE, true, 100.0, true, 2000.0);
    g_hCvarRandBonus = CreateConVar(                        "rand_bonus",                    "1",       "Bonus mode: 0 = base, no variation; 1 = variance from base; 2 = full random.", FCVAR_NONE, true, 0.0, true, 2.0);
    g_hCvarRandBonusBase = CreateConVar(                    "rand_bonus_base",               "1.0",     "Bonus base value: 0.0 = static; anything else: scaled to X times distance.", FCVAR_NONE, true, 0.0, true, 5.0);
    g_hCvarRandBonusStatic = CreateConVar(                  "rand_bonus_static",           "400",       "Bonus static value (used only if bonus_base is 0.0).", FCVAR_NONE, true, 0.0, true, 1600.0);
    g_hCvarRandBonusVar = CreateConVar(                     "rand_bonus_var",                "0.2",     "For bonus variance: how much it can deviate from normal value.", FCVAR_NONE, true, 0.1, true, 0.9);
    g_hCvarRandBonusMin = CreateConVar(                     "rand_bonus_min",               "200",      "For fully random bonus: the minimum.", FCVAR_NONE, true, 50.0, true, 900.0);
    g_hCvarRandBonusMax = CreateConVar(                     "rand_bonus_max",               "800",      "For fully random bonus: the maximum.", FCVAR_NONE, true, 100.0, true, 4000.0);
    g_hCvarRandBonusSolidFactor = CreateConVar(             "rand_bonus_solid_factor",        "2.0",    "How much solid health is worth for the damage bonus.", FCVAR_NONE, true, 0.0);
    g_hCvarRandBonusScaleMode = CreateConVar(               "rand_bonus_scale_mode",          "2",      "1 = scale; 2 = reduce.", FCVAR_NONE, true, 0.0);
    g_hCvarStaticBonus = CreateConVar(                      "rand_bonus_static_surv",        "25",      "Static survival bonus (on top of damage bonus).", FCVAR_NONE, true, 0.0);

    g_hCvarAmmoAk = CreateConVar(                           "rand_ammo_ak",                 "250",      "Ammo for the AK47 in Random.", FCVAR_NONE, true, 0.0, false);
    g_hCvarAmmoScout = CreateConVar(                        "rand_ammo_scout",              "150",      "Ammo for the Scout rifle in Random.", FCVAR_NONE, true, 0.0, false);
    g_hCvarAmmoAWP = CreateConVar(                          "rand_ammo_awp",                 "90",      "Ammo for the AWP rifle in Random.", FCVAR_NONE, true, 0.0, false);
    g_hCvarAmmoSniper = CreateConVar(                       "rand_ammo_sniper",              "60",      "Ammo for the military sniper rifle in Random.", FCVAR_NONE, true, 0.0, false);

    g_hCvarColorMinitank = CreateConVar(                    "rand_color_minitank",     "180 180 0",     "Color for minitanks.", FCVAR_NONE);

    g_hArCvarWeight[INDEX_NOITEM] = CreateConVar(           "rand_weight_nothing",          "25",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_PISTOL] = CreateConVar(           "rand_weight_pistol",           "13",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_T1SMG] = CreateConVar(            "rand_weight_t1smg",            "16",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_T1SHOTGUN] = CreateConVar(        "rand_weight_t1shotgun",        "16",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_T2RIFLE] = CreateConVar(          "rand_weight_t2rifle",           "3",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_T2SHOTGUN] = CreateConVar(        "rand_weight_t2shotgun",         "3",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_SNIPER] = CreateConVar(           "rand_weight_sniper",            "6",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_MELEE] = CreateConVar(            "rand_weight_melee",            "14",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_T3] = CreateConVar(               "rand_weight_t3",                "3",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_CANISTER] = CreateConVar(         "rand_weight_canister",         "18",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_PILL] = CreateConVar(             "rand_weight_pill",             "16",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_THROWABLE] = CreateConVar(        "rand_weight_throwable",         "8",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_KIT] = CreateConVar(              "rand_weight_kit",              "11",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_UPGRADE] = CreateConVar(          "rand_weight_upgrade",           "5",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_AMMO] = CreateConVar(             "rand_weight_ammo",              "5",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_JUNK] = CreateConVar(             "rand_weight_junk",             "18",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_SILLY] = CreateConVar(            "rand_weight_silly",             "3",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarWeight[INDEX_GIFT] = CreateConVar(             "rand_weight_gift",              "3",       "Weight for picking item spawns.",          FCVAR_NONE, true, 0.0, true, 100.0 );

    g_hArCvarSurvWeight[INDEX_SURV_NOTHING] = CreateConVar( "rand_weight_surv_nothing",      "5",       "Weight for picking survivor starting gear.", FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarSurvWeight[INDEX_SURV_PISTOL] = CreateConVar(  "rand_weight_surv_pistol",      "15",       "Weight for picking survivor starting gear.", FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarSurvWeight[INDEX_SURV_DUALS] = CreateConVar(   "rand_weight_surv_duals",       "20",       "Weight for picking survivor starting gear.", FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarSurvWeight[INDEX_SURV_MAGNUM] = CreateConVar(  "rand_weight_surv_magnum",      "10",       "Weight for picking survivor starting gear.", FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarSurvWeight[INDEX_SURV_T1SMG] = CreateConVar(   "rand_weight_surv_t1smg",       "40",       "Weight for picking survivor starting gear.", FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarSurvWeight[INDEX_SURV_T1SHOT] = CreateConVar(  "rand_weight_surv_t1shotgun",   "40",       "Weight for picking survivor starting gear.", FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarSurvWeight[INDEX_SURV_MELEE] = CreateConVar(   "rand_weight_surv_melee",       "10",       "Weight for picking survivor starting gear.", FCVAR_NONE, true, 0.0, true, 100.0 );

    g_hArCvarEvtWeight[EVT_ITEM] = CreateConVar(            "rand_weight_evt_item",          "3",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_HORDE_HUGE] = CreateConVar(      "rand_weight_evt_horde",         "2",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_HORDE_NONE] = CreateConVar(      "rand_weight_evt_nohorde",       "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_UNCOMMON] = CreateConVar(        "rand_weight_evt_uncommon",      "3",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_CLOWNS] = CreateConVar(          "rand_weight_evt_circus",        "2",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_DOORS] = CreateConVar(           "rand_weight_evt_doors",         "3",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_QUADS] = CreateConVar(           "rand_weight_evt_quads",         "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_WEATHER] = CreateConVar(         "rand_weight_evt_storm",         "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_FOG] = CreateConVar(             "rand_weight_evt_fog",           "5",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_ABUNDANCE] = CreateConVar(       "rand_weight_evt_abundance",     "3",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_SNIPER] = CreateConVar(          "rand_weight_evt_sniper",        "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_GIFTS] = CreateConVar(           "rand_weight_evt_gifts",         "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_DEFIB] = CreateConVar(           "rand_weight_evt_defib",         "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_ADREN] = CreateConVar(           "rand_weight_evt_adren",         "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_NOHUD] = CreateConVar(           "rand_weight_evt_nohud",         "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_L4D1] = CreateConVar(            "rand_weight_evt_l4d1",          "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_FF] = CreateConVar(              "rand_weight_evt_ff",            "3",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_SILENCE] = CreateConVar(         "rand_weight_evt_sound",         "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_PEN_ITEM] = CreateConVar(        "rand_weight_evt_penitem",       "2",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_PEN_HEALTH] = CreateConVar(      "rand_weight_evt_penhealth",     "3",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_PEN_M2] = CreateConVar(          "rand_weight_evt_penm2",         "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_PEN_TIME] = CreateConVar(        "rand_weight_evt_pentime",       "2",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_MINITANKS] = CreateConVar(       "rand_weight_evt_minitanks",     "5",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_KEYMASTER] = CreateConVar(       "rand_weight_evt_keymaster",     "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_BADCOMBO] = CreateConVar(        "rand_weight_evt_badcombo",      "3",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_PROTECT] = CreateConVar(         "rand_weight_evt_protect",       "5",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_ENCUMBERED] = CreateConVar(      "rand_weight_evt_encumbered",    "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_BOOBYTRAP] = CreateConVar(       "rand_weight_evt_boobytrap",     "2",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_SKEET] = CreateConVar(           "rand_weight_evt_skeet",         "5",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_FIREPOWER] = CreateConVar(       "rand_weight_evt_firepower",     "2",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_AMMO] = CreateConVar(            "rand_weight_evt_ammo",          "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_WOMEN] = CreateConVar(           "rand_weight_evt_women",         "2",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_GUNSWAP] = CreateConVar(         "rand_weight_evt_gunswap",       "5",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_WITCHES] = CreateConVar(         "rand_weight_evt_witches",       "5",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_BADSANTA] = CreateConVar(        "rand_weight_evt_badgifts",      "3",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_MEDIC] = CreateConVar(           "rand_weight_evt_medic",         "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarEvtWeight[EVT_BOOMFLU] = CreateConVar(         "rand_weight_evt_boomerflu",     "4",       "Weight for picking special event.",        FCVAR_NONE, true, 0.0, true, 100.0 );
    ...
    g_hArCvarGiftWeight[GIFT_POS_AMMO] = CreateConVar(      "rand_weight_gift_getammo",      "3",       "Weight for picking gift effects.",         FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarGiftWeight[GIFT_POS_ITEMS] = CreateConVar(     "rand_weight_gift_getitems",     "7",       "Weight for picking gift effects.",         FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarGiftWeight[GIFT_POS_LASER] = CreateConVar(     "rand_weight_gift_getlaser",     "2",       "Weight for picking gift effects.",         FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarGiftWeight[GIFT_POS_ALARMS] = CreateConVar(    "rand_weight_gift_alarmsoff",    "1",       "Weight for picking gift effects.",         FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarGiftWeight[GIFT_POS_INSIGHT] = CreateConVar(   "rand_weight_gift_survinsight",  "2",       "Weight for picking gift effects.",         FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarGiftWeight[GIFT_NEG_VOMIT] = CreateConVar(     "rand_weight_gift_vomit",        "3",       "Weight for picking gift effects.",         FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarGiftWeight[GIFT_NEG_PANIC] = CreateConVar(     "rand_weight_gift_panic",        "2",       "Weight for picking gift effects.",         FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarGiftWeight[GIFT_NEG_ALLDROP] = CreateConVar(   "rand_weight_gift_alldrop",      "2",       "Weight for picking gift effects.",         FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarGiftWeight[GIFT_NEG_EXPLODE] = CreateConVar(   "rand_weight_gift_explode",      "3",       "Weight for picking gift effects.",         FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarGiftWeight[GIFT_NEG_FIRE] = CreateConVar(      "rand_weight_gift_fire",         "4",       "Weight for picking gift effects.",         FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarGiftWeight[GIFT_NEG_BLIND] = CreateConVar(     "rand_weight_gift_blind",        "3",       "Weight for picking gift effects.",         FCVAR_NONE, true, 0.0, true, 100.0 );
    g_hArCvarGiftWeight[GIFT_NEG_INSIGHT] = CreateConVar(   "rand_weight_gift_infinsight",   "2",       "Weight for picking gift effects.",         FCVAR_NONE, true, 0.0, true, 100.0 );
